...
describe('Starting tests for supplychain_app', function() {
  ...
  it('website title should be supplychain_app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('supplychain_app');
    })
  });
  ...
  it('navbar-brand should be supplychain_app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('supplychain_app');
    });
  });
